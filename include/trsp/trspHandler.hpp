...
class TrspHandler : public pgrouting::Pgr_messages {
   ...
   * - Position::ILLEGAL: Its not a predecessor of anything
   *
 ...
 public:
  TrspHandler(std::vector<Edge_t> &edges, const std::vector<Edge_t> &new_edges,
              const bool directed, const std::vector<Rule> &ruleList);

  TrspHandler(std::vector<Edge_t> &edges, const bool directed,
 ...
 private:
  void construct_graph(const std::vector<Edge_t> &, const std::vector<Edge_t> &,
                       const bool);
