...
TrspHandler::TrspHandler(std::vector<Edge_t> &edges, const bool directed,
...
TrspHandler::TrspHandler(std::vector<Edge_t> &edges,
                         const std::vector<Edge_t> &new_edges,
                         const bool directed, const std::vector<Rule> &ruleList)
    : m_ruleTable() {
  initialize_restrictions(ruleList);
...
// -------------------------------------------------------------------------
void TrspHandler::renumber_edges(std::vector<Edge_t> &edges) {
  int64_t idx(0);
  for (auto &e : edges) {
...
double TrspHandler::construct_path(int64_t ed_id, Position pos) {
  ...
  double ret = construct_path(
      static_cast<int64_t>(
          m_parent[static_cast<size_t>(ed_id)].e_idx[static_cast<size_t>(pos)]),
      static_cast<Position>(m_parent[static_cast<size_t>(ed_id)]
                                .v_pos[static_cast<size_t>(pos)]));
  Path_t pelement = {};
  auto cur_edge = &m_edges[static_cast<size_t>(ed_id)];
  if (pos == Position::RC_EDGE) {
    pelement.node = cur_edge->startNode();
...
// -------------------------------------------------------------------------
double TrspHandler::getRestrictionCost(int64_t edge_ind, const EdgeInfo &edge,
                                       bool isStart) {
  double cost = 0.0;
  ...
  for (const auto &rule : vecRules) {
    bool flag = true;
    auto v_pos = (isStart ? Position::C_EDGE : Position::RC_EDGE);
    ...
    for (auto const &precedence : rule.precedencelist()) {
      ...
      auto m_parent_ind = m_parent[static_cast<size_t>(edge_ind)]
                              .e_idx[static_cast<size_t>(v_pos)];
      v_pos = m_parent[static_cast<size_t>(edge_ind)]
                  .v_pos[static_cast<size_t>(v_pos)];
...
double TrspHandler::get_tot_cost(double cost, size_t edge_idx, bool isStart) {
  if (isStart) {
    return m_dCost[edge_idx].startCost + cost;
  }
  return m_dCost[edge_idx].endCost + cost;
}

// -------------------------------------------------------------------------
void TrspHandler::explore(int64_t cur_node, const EdgeInfo cur_edge,
                          bool isStart) {
  ...
  for (const auto &index : vecIndex) {
    ...
    auto extra_cost =
        getRestrictionCost(static_cast<int64_t>(cur_edge.idx()), edge, isStart);

    if ((edge.startNode() == cur_node) && (edge.cost() >= 0.0)) {
      double totalCost =
          get_tot_cost(edge.cost() + extra_cost, cur_edge.idx(), isStart);

      if (totalCost < m_dCost[index].endCost) {
        m_dCost[index].endCost = totalCost;
        m_parent[edge.idx()].v_pos[static_cast<size_t>(Position::RC_EDGE)] =
            isStart ? Position::C_EDGE : Position::RC_EDGE;
        m_parent[edge.idx()].e_idx[static_cast<size_t>(Position::RC_EDGE)] =
            cur_edge.idx();
    ...
    if ((edge.endNode() == cur_node) && (edge.r_cost() >= 0.0)) {
      double totalCost =
          get_tot_cost(edge.r_cost() + extra_cost, cur_edge.idx(), isStart);

      if (totalCost < m_dCost[index].startCost) {
        m_dCost[index].startCost = totalCost;
        m_parent[edge.idx()].v_pos[static_cast<size_t>(Position::C_EDGE)] =
            isStart ? Position::C_EDGE : Position::RC_EDGE;
        m_parent[edge.idx()].e_idx[static_cast<size_t>(Position::C_EDGE)] =
            cur_edge.idx();
...
// -------------------------------------------------------------------------
int TrspHandler::initialize_restrictions(const std::vector<Rule> &ruleList) {
 ...
 *
 */
Path TrspHandler::process(const int64_t start_vertex,
 ...
 */
std::deque<Path>
TrspHandler::process(const std::map<int64_t, std::set<int64_t>> &combinations) {
...
void TrspHandler::initialize_que() {
  ...
  for (const auto source : m_adjacency[m_start_vertex]) {
    ...
    if (cur_edge.startNode() == m_start_vertex && cur_edge.cost() >= 0.0) {
      ...
      m_parent[cur_edge.idx()].v_pos[0] = Position::ILLEGAL;
    ...
    if (cur_edge.endNode() == m_start_vertex && cur_edge.r_cost() >= 0.0) {
      ...
      m_parent[cur_edge.idx()].v_pos[1] = Position::ILLEGAL;
...
Path TrspHandler::process_trsp(size_t edge_count) {
  pgassert(m_path.start_id() == m_start_vertex);
  ...
  construct_path(static_cast<int64_t>(cur_edge.idx()),
                 m_current_node == cur_edge.startNode() ? Position::C_EDGE
                                                        : Position::RC_EDGE);
...
void TrspHandler::construct_graph(const std::vector<Edge_t> &edges,
                                  const std::vector<Edge_t> &new_edges,
                                  const bool directed) {
...
// -------------------------------------------------------------------------
void TrspHandler::connectStartEdge(size_t firstEdge_idx,
...
// -------------------------------------------------------------------------
void TrspHandler::connectEndEdge(size_t firstEdge_idx, size_t secondEdge_idx) {
  EdgeInfo &firstEdge = m_edges[firstEdge_idx];
  EdgeInfo &secondEdge = m_edges[secondEdge_idx];
  ...
  if (firstEdge.endNode() == secondEdge.endNode() && secondEdge.cost() >= 0.0) {
    secondEdge.connect_endEdge(firstEdge_idx);
  }
}
