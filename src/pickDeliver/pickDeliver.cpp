...
void Pgr_pickDeliver::solve() {
  auto initial_sols = solutions;

  if (m_initial_id == 0) {
    msg.log << "trying all \n";
    for (int i = 1; i < 7; ++i) {
      initial_sols.push_back(
          Initial_solution((Initials_code)i, m_orders.size()));
    ...
    initial_sols.push_back(
        Initial_solution((Initials_code)m_initial_id, m_orders.size()));
    // TODO(vicky) calculate the time it takes
...
std::vector<Schedule_rt> Pgr_pickDeliver::get_postgres_result() const {
...
void Pgr_pickDeliver::add_node(const Vehicle_node &node) {
 ...
 *
 */
Pgr_pickDeliver::Pgr_pickDeliver(const std::vector<Orders_t> &pd_orders,
                                 const std::vector<Vehicle_t> &vehicles,
                                 const pgrouting::tsp::Dmatrix &cost_matrix,
                                 double factor, size_t p_max_cycles,
                                 int initial)
    : PD_problem(this), m_initial_id(initial), m_max_cycles(p_max_cycles),
      m_nodes(), m_cost_matrix(cost_matrix), m_orders(pd_orders),
  ...
  if (!(m_initial_id > 0 &&
        m_initial_id < static_cast<int>(Initials_code::OneDepot))) {
    msg.log << "\n m_initial_id " << m_initial_id;
  }
  pgassertwm(m_initial_id > 0 &&
                 m_initial_id <= static_cast<int>(Initials_code::OneDepot),
             msg.get_log().c_str());
  ...
  for (const auto &o : m_orders) {
    if (!m_trucks.is_order_ok(o)) {
      ...
      msg.log << "The order " << o.id() << " is not feasible on any truck";
      msg.log << "\n" << o;
      return;
    }
  }
